...
${sourceCode}`;
  ...
  const runAiAnalysis = async (code, language) => {
    ...
    try {
      ...
      // DeepSeek V3 and R1 confirmed permanently free per OpenRouter announcement.
      // openrouter/cypher-alpha is OpenRouter's own free model — very reliable.
      const FREE_MODELS = [
        'deepseek/deepseek-r1:free',                    // #1 confirmed permanently free, best reasoning
        'deepseek/deepseek-chat:free',                  // #2 confirmed permanently free, DeepSeek V3
        'openrouter/cypher-alpha:free',                 // #3 OpenRouter's own model, very reliable
        'meta-llama/llama-3.3-70b-instruct:free',       // #4 confirmed free, reliable fallback
        'openrouter/free',                              // #5 last resort auto-picker
      ];
      ...
      const messages = [
        {
          role: "system",
          content: "You are a code analysis tool. You only output valid JSON arrays. Never output explanations, markdown, or any text outside the JSON array."
        },
        {
          role: "user",
          content: `Analyze this ${language} code and return a JSON array of issues found.

Each issue must be: {"severity":"critical|high|medium|low","category":"security|runtime|performance|quality","message":"description","line":0}
