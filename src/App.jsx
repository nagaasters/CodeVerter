...
${sourceCode}`;
  ...
  const runAiAnalysis = async (code, language) => {
    ...
    try {
      ...
      const prompt = `You are a senior code reviewer. Analyze this ${language} code carefully and find ALL bugs, errors, and issues.

Look specifically for:
- Syntax errors (missing colons, invalid operators like ===, assignment in conditions like if a = 10)
- Runtime errors (division by zero, type mismatches, undefined variables, key errors)
- Logic bugs (mutable default arguments, wrong method calls like .append = instead of .append())
- Security issues (missing file close, hardcoded credentials, unsafe operations like pickle/eval)
- Performance issues (infinite recursion, blocking calls in async, memory leaks)
- Bad practices (shadowing builtins, bare except, threads not joined, shared class mutables)

Return ONLY a raw JSON array. No explanation, no markdown, no code blocks. Just the array.
Each item must have exactly these fields:
{"severity":"critical|high|medium|low","category":"security|runtime|performance|quality","message":"what is wrong and why","line":0}
