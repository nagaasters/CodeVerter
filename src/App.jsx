...
${sourceCode}`;
  ...
  const runAiAnalysis = async (code, language) => {
    ...
    try {
      // Ordered best → worst for code analysis based on Feb 2026 benchmarks.
      // Each model is tried in order — first one that responds wins.
      const FREE_MODELS = [
        'openai/gpt-oss-120b:free',                     // #1 OpenAI open-weight, near o4-mini reasoning
        'meta-llama/llama-4-maverick:free',             // #2 Llama 4 Maverick — top free coding model
        'deepseek/deepseek-r1:free',                    // #3 DeepSeek R1 — strong reasoning/code
        'deepseek/deepseek-r1-distill-llama-70b:free',  // #4 DeepSeek R1 distill
        'meta-llama/llama-4-scout:free',                // #5 Llama 4 Scout — large context
        'meta-llama/llama-3.3-70b-instruct:free',       // #6 Llama 3.3 70B — reliable fallback
        'google/gemini-2.0-flash-exp:free',             // #7 Gemini 2.0 Flash
        'openrouter/free',                              // #8 last resort — auto-picks whatever is up
      ];

      // Ordered best → worst for code analysis.
      // DeepSeek V3 and R1 are confirmed permanently free by OpenRouter.
      // openrouter/free is last resort — picks randomly so quality varies.
      const FREE_MODELS = [
        'deepseek/deepseek-r1:free',                    // #1 best reasoning, confirmed free
        'deepseek/deepseek-chat:free',                  // #2 DeepSeek V3, confirmed free
        'deepseek/deepseek-r1-distill-llama-70b:free',  // #3 R1 distill, confirmed free
        'meta-llama/llama-3.3-70b-instruct:free',       // #4 reliable llama fallback
        'openrouter/free',                              // #5 last resort auto-picker
      ];

      const prompt = `Analyze this ${language} code for bugs and issues. You MUST respond with ONLY a JSON array and nothing else. No text before or after. No markdown. No explanation. Start your response with [ and end with ].

Find: syntax errors, runtime errors, logic bugs, security issues, performance problems, bad practices.

Required format - respond with ONLY this structure:
[{"severity":"critical","category":"security","message":"description","line":5},{"severity":"high","category":"runtime","message":"description","line":12}]

Severity must be: critical, high, medium, or low
Category must be: security, runtime, performance, or quality
...
Respond with ONLY the JSON array starting with [:`;
      ...
      for (const model of FREE_MODELS) {
        try {
          ...
          if (arrayStart === -1 || arrayEnd === -1) { lastError = 'No JSON array in response'; continue; }

          const findings = JSON.parse(clean.slice(arrayStart, arrayEnd + 1));
          const usedModel = data.model || model;
          setAiModel(usedModel);
          setAiAnalysis(Array.isArray(findings) ? findings : []);
          return; // success
      ...
      // All models failed
      setAiAnalysis([{ severity: 'low', category: 'quality', message: `AI analysis unavailable — all free models failed. Try again later. (${lastError})`, line: 0 }]);

    } catch (err) {
      // Safety net — catches any unexpected error outside the loop
      setAiAnalysis([{ severity: 'low', category: 'quality', message: `AI analysis error: ${err.message}`, line: 0 }]);
    } finally {
