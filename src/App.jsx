        ...
        <div className="max-w-7xl mx-auto">
          {activeTab === 'converter' ? (
            ...
            <>
                  ...
                  <div className="h-[600px] overflow-y-auto p-4">
                    {isChecking ? (
                      ...
                      <div className="space-y-4">
                        ...
                        {/* AI Deep Analysis — shown first, above all pattern findings */}
                        {(isAiAnalyzing || aiAnalysis || aiError) && (
                          <div className="bg-gray-800/40 border border-cyan-700/30 rounded-xl p-4">
                            ...
                            {isAiAnalyzing && (
                              <div className="text-xs text-cyan-300">AI is reviewing your code for deeper issues...</div>
                            )}
                            ...
                            {!isAiAnalyzing && aiAnalysis && aiAnalysis.length === 0 && (
                              <div className="text-xs text-green-300">✓ AI found no additional issues.</div>
                            )}
                            {!isAiAnalyzing && aiAnalysis && aiAnalysis.length > 0 && (
                                    ...
                                    <div className="flex items-start gap-2">
                                      ...
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                          ...
                                          <button
                                            onClick={(e) => { e.stopPropagation(); fixIssues(idx); }}
                                            disabled={isFixing}
                                        ...
                                        {issue.line > 0 && <p className="text-xs text-gray-400 mt-1">Line {issue.line}</p>}
                                      </div>
                                    </div>
                                  </div>
                                ))}
              ...
              {/* Fixed Code Output — full width below code + results */}
              {(isFixing || fixedCode || fixResults?.error) && (
                <div className="mt-6 bg-gray-800/50 backdrop-blur-sm rounded-xl overflow-hidden border border-emerald-700/50 shadow-xl">
                  <div className="bg-emerald-900/40 px-4 py-3 flex items-center justify-between border-b border-emerald-700/40">
                    ...
                    {fixedCode && (
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-gray-400">Fixed by Pattern Detection + AI · <span className="font-mono text-cyan-500">{fixResults?.model}</span></span>
                        <button
                  ...
                  {isFixing && (
                    <div className="flex items-center justify-center p-10">
                      ...
                      <span className="text-sm text-gray-300">AI is applying fixes...</span>
                    </div>
                  )}
                  {fixResults?.error && (
                  ...
                  {fixedCode && (
                    <>
                      <pre className="p-4 text-sm text-gray-200 font-mono overflow-x-auto overflow-y-auto bg-gray-900/50 whitespace-pre-wrap max-h-[400px]">{fixedCode}</pre>
                      <div className="bg-gray-800/50 px-4 py-3 border-t border-emerald-700/40 flex items-center gap-2 flex-wrap">
                        <span className="text-xs text-gray-400">Issues fixed:</span>
                        {fixResults?.issues?.map((issue, i) => (
                          <span key={i} className="text-xs px-2 py-0.5 bg-emerald-900/50 text-emerald-300 rounded">
                            {issue.source === 'ai' ? '🤖' : '🔍'} {issue.category} line {issue.line}
                          </span>
                        ))}
                      </div>
                    </>
