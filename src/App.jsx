...
const CodeVerter = () => {
  ...
  const clickIssue = (line) => {
    ...
    // Scroll the code editor to that line
    if (codeEditorRef.current) {
      const lineHeight = 20; // px per line at text-sm
...
${sourceCode}`;
  ...
  const runAiAnalysis = async (code, language) => {
    ...
    try {
      // Confirmed working free models from OpenRouter - Feb 2026
      const FREE_MODELS = [
        'meta-llama/llama-4-maverick:free',              // #1 confirmed working
        'meta-llama/llama-4-scout:free',                 // #2 confirmed working
        'meta-llama/llama-3.3-70b-instruct:free',        // #3 confirmed working
        'moonshotai/kimi-vl-a3b-thinking:free',          // #4 confirmed working
        'openrouter/free',                               // #5 auto-picks any available
      ];
...
Return ONLY the fixed code with no explanation, no markdown, no code blocks. Just the raw fixed code:`;
    ...
    try {
      for (const model of ['meta-llama/llama-4-maverick:free', 'meta-llama/llama-4-scout:free', 'meta-llama/llama-3.3-70b-instruct:free', 'openrouter/free']) {
        try {
          const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            method: "POST",
            headers: {
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl overflow-hidden border border-gray-700/50 shadow-xl">
                  ...
                  <div className="relative flex h-[600px] font-mono text-sm overflow-hidden">
                    {/* Line numbers */}
                    <div className="select-none bg-gray-900/60 text-gray-600 text-right px-3 py-4 overflow-hidden min-w-[3rem] leading-5">
                      {(checkCode || ' ').split('\n').map((_, i) => (
                        <div
                          key={i}
                          className={`leading-5 cursor-pointer transition-colors ${highlightedLine === i + 1 ? 'text-red-400 font-bold' : 'hover:text-gray-400'}`}
                          onClick={() => clickIssue(i + 1)}
                        >
                          {i + 1}
                        </div>
                      ))}
                    </div>
                    {/* Highlighted line overlay */}
                    <div className="absolute left-12 right-0 pointer-events-none overflow-hidden" style={{ top: 0 }}>
                      {highlightedLine && (
                        <div
                          ...
                          <div className="bg-gray-800/60 border border-emerald-700/40 rounded-xl p-4">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <button
                                ...
                                disabled={isFixing || (!aiAnalysis || aiAnalysis.length === 0)}
                            ...
                            {(!aiAnalysis || aiAnalysis.length === 0) && !isAiAnalyzing && (
                              <p className="text-xs text-gray-500">
                                {isAiAnalyzing ? 'Waiting for AI analysis...' : aiError ? 'AI unavailable — fix requires AI deep analysis to complete first.' : 'Run analysis to enable fixes.'}
                              </p>
                            )}
                          </div>
                        )}
