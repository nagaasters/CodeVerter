...
- Security issues (missing file close, hardcoded values, unsafe operations)
- Performance issues (infinite recursion, inefficient patterns)
- Bad practices (shadowing builtins like list/dict, bare except, invalid raise statements)
...
JSON array only:`;
      ...
      // Show API error in UI instead of silently failing
      if (data.error) {
        setAiAnalysis([{ severity: 'low', category: 'quality', message: `AI analysis unavailable: ${data.error.message}`, line: 0 }]);
        return;
      }
      ...
      if (arrayStart === -1 || arrayEnd === -1) {
        setAiAnalysis([{ severity: 'low', category: 'quality', message: 'AI returned an unexpected response format. Try again.', line: 0 }]);
        return;
      }
      ...
      setAiAnalysis([{ severity: 'low', category: 'quality', message: `AI analysis failed: ${err.message}`, line: 0 }]);
    } finally {
      setIsAiAnalyzing(false);
    }
  };
