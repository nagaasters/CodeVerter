...
${sourceCode}`;
  ...
  const runAiAnalysis = async (code, language) => {
    ...
    // Fallback chain — tries each free model in order until one succeeds
    // Model IDs confirmed available as of Feb 2026 on OpenRouter free tier
    const FREE_MODELS = [
      'openrouter/free',                              // auto-picks any available free model
      'deepseek/deepseek-chat-v3-0324:free',          // DeepSeek V3 — confirmed free
      'deepseek/deepseek-r1-0528:free',               // DeepSeek R1 — confirmed free
      'deepseek/deepseek-r1-distill-llama-70b:free',  // DeepSeek R1 distill — confirmed free
      'meta-llama/llama-3.3-70b-instruct:free',       // Llama 3.3 70B — confirmed free
    ];
...
JSON array only:`;
    ...
    for (const model of FREE_MODELS) {
      try {
        ...
        // If this model errored, try the next one
        if (data.error) {
        ...
        // OpenRouter returns the actual model used in data.model (useful when using openrouter/free)
